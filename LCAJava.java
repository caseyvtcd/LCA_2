...
class LCAJava
{
	...
	public static boolean isNodePresent(Node root, Node node)
	{
		...
		return isNodePresent(root.Successors[0], node) || isNodePresent(root.Successors[1], node);
	}
...
//		boolean left = findLCA(root.left, lca, x, y);
		boolean left = findLCA(root.Successors[0], lca, x, y);

		// recursively check if x or y exists in the right subtree
//		boolean right = findLCA(root.right, lca, x, y);
		boolean right = findLCA(root.Successors[1], lca, x, y);
